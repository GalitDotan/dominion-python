...
from app.crud.profile import get_profile_for_username
from app.db.mongodb import AsyncIOMotorClient
from app.models.profile import ProfileInFollow
...
async def get_followers_count(conn: AsyncIOMotorClient, current_username: str) -> int:
    follower_count = await conn[database_name][followers_collection_name].count_documents(
        {"following": current_username})
    return follower_count


async def get_following_count(conn: AsyncIOMotorClient, current_username: str) -> int:
    following_count = await conn[database_name][followers_collection_name].count_documents(
