...
from app.core.config import database_name, profiles_collection_name
from app.db.mongodb import AsyncIOMotorClient
from app.models.profile import Country, ProfileInUpdate, ProfileInDB
...
async def update_profile_for_username(
    ...
    await conn[database_name][profiles_collection_name].update_one({"username": old_profile.username},
...
async def add_points_to_profile(conn: AsyncIOMotorClient, current_username: str, points: int):
    try:
        await conn[database_name][profiles_collection_name].update_one({"username": current_username},
                                                                       {'$inc': {"points": points}})
    except Exception as e:
        raise HTTPException(
            status_code=HTTP_422_UNPROCESSABLE_ENTITY,
