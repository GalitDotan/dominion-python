...
class Game(object):
    ...
    def run(self):
        ...
        """
        self.log.info("Available cards: " + str(self.card_counts))
        game = self
        while not game.over():
            game = game.take_turn()
...
class PlayerState(object):
    ...
    def __init__(self, player, hand, drawpile, discard, tableau, actions=0,
        ...
        assert isinstance(self.actions, int)
        self.buys = buys
        assert isinstance(self.buys, int)
        self.coins = coins
        assert isinstance(self.coins, int)
        self.hand = hand
        assert isinstance(self.hand, tuple)
        self.drawpile = drawpile
        assert isinstance(self.drawpile, tuple)
        self.discard = discard
        assert isinstance(self.discard, tuple)
        self.tableau = tableau
        assert isinstance(self.tableau, tuple)
        # TODO: duration cards
